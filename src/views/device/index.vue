  ...
  <PageWrapper :content-style="{ backgroundColor: '#fff', padding: '30px' }">
    ...
    <Tabs v-if="!pageType" v-model:active-key="tabKey" :destroy-inactive-tab-pane="true">
      <TabPane tab="视频设备" key="HDMI"
        ><VideoTable :isTask="false" :type="tabKey" @pageChange="handlePageChange"
      /></TabPane>
      <TabPane tab="音频设备" key="AUDIO"
        ><AudioTable :isTask="false" :type="tabKey" @pageChange="handlePageChange"
      /></TabPane>
    </Tabs>
    <AddChannel
      v-if="pageType === 'add'"
      :type="tabKey"
      :channels="alreadyAddChannel"
