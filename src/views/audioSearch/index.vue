<template>
  <PageWrapper :content-style="{ backgroundColor: '#fff', padding: '30px' }">
    ...
    <Preview @register="registerModal" type="audio" />
  </PageWrapper>
</template>
<script lang="ts" setup>
  ...
  // service
  import { delAudio, downloadAudio, getAudioSearch } from '/@/api/search';
  ...
  async function handleDownload(item, type) {
    ...
    // });
    // const result = await downloadAudio({ fileList });
    // downloadByData(result, 'audio.xlsx');
    const url =
      origin +
      `/api/search/download/batch?channel=${item.channel}&date=${item.fileTime}&aiType=VOICE&fileFormat=${type}`;
    downloadByUrl2(
      url,
      type === 'mp4'
        ? `audio-${item.channel}-${item.fileTime}.zip`
  ...
  async function handleBatchDownload(type) {
    ...
    tableData.map((a) => {
      ...
      const url =
        origin +
        `/api/search/download/batch?channel=${a.channel}&date=${a.fileTime}&aiType=VOICE&fileFormat=${type}`;
      downloadByUrl2(url, type === 'mp4' ? 'audio.zip' : 'ts.zip');
    });
  ...
  async function handleDel(item) {
    await delAudio({ ids: item.idList });
    reload();
  }
</script>
