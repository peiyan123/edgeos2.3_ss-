 ...
 * 图形选择功能
 */
export function ShapeSelectDecorators<T extends new (...args: any[]) => AiMarkInterface>(
  constructor: T,
) {
    ...
    drawSelectedShape() {
      this.canvasOperation((ctx: CanvasRenderingContext2D) => {
        if (this.shapeSelected) {
          ...
          // 绘制成矩形
          ctx.fillRect(
            this.shapeSelected.minX!,
            this.shapeSelected.minY!,
            this.shapeSelected.maxX! - this.shapeSelected.minX!,
            this.shapeSelected.maxY! - this.shapeSelected.minY!,
          );
          ctx.strokeRect(
            this.shapeSelected.minX!,
            this.shapeSelected.minY!,
      ...
      this.destroy.push(
        merge(
          ...
          fromEvent(document, 'keydown').pipe(
            filter((e: any) => this.shapeSelectedExitKeyEsc! && e.keyCode === 27),
          ),
        ).subscribe(() => {
          if (this.shapeSelected) {
