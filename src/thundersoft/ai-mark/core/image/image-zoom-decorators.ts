...
export function ImageZoomDecorators<T extends new (...args: any[]) => AiMarkInterface>(
  constructor: T,
) {
    ...
    canvasZoomScroll() {
      this.destroy.push(
        this.canvasElementEvents!.scroll.subscribe((e) => {
          ...
          //小：不能小于 100像素
          const imageMaxEdge =
            this.imageAspectRatio! > this.accuracy ? this.image!.width : this.image!.height;
          if (this.canvasZoom * zoom * imageMaxEdge < this.accuracy * 100) {
            return;
          }

          // 图片渲染开始点计算
          const xDistance = currentPoint.x - this.imageStartPoint!.x;
          const yDistance = currentPoint.y - this.imageStartPoint!.y;
