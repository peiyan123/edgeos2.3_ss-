 ...
 * PointDragDecorators
 */
export function PointDragDecorators<T extends new (...args: any[]) => AiMarkInterface>(
  constructor: T,
) {
      ...
      this.destroy.push(
        this.canvasElementEvents!.mousemove.subscribe((e) => {
          ...
          if (this.pointDragDrawing && this.pointDragingmouseDown) {
            ...
            // 新点的位置
            const x =
              this.pointDragingmouseDownPoint!.x! + (e.screenX - this.pointDragingmouseDown.x);
            const y =
              this.pointDragingmouseDownPoint!.y! + (e.screenY - this.pointDragingmouseDown.y);
