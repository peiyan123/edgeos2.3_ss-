 ...
 */
export function ShapeSegmentationDecorators<T extends new (...args: any[]) => AiMarkInterface>(
  constructor: T,
) {
    ...
    shapeSegmentationDrawing(option: ShapeSegmentationData) {
      ...
      this.destroyMap.set('ShapeSegmentationDecorators1', [
        this.pointsCreateByMouseClickBeforeEvent!.subscribe((point) => {
          const bool = this.shapeSegmentations?.some((item) => {
            return item.shapeId == point.source.shapeId;
          });
          if (bool) return;
